...
const Galeria = () => {
    ...
    const listaFotos: FotoMomento[] = [
        {
            src: "/imagens/galeria01.jpeg", // ESTA FOTO TERÁ DESTAQUE NO DESKTOP
        ...
        {
            src: "/imagens/galeria05.jpeg",
            alt: "Desenvolvimento infantil",
            legenda: "Evolução constante em cada etapa."
        }
    ];
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 pb-12 border-b border-[#EEDDF6]/50">
                    <div className="max-w-4xl space-y-4 md:space-y-6">
                        ...
                        <p className="text-gray-500 text-base md:text-lg leading-relaxed max-w-2xl">
                            Um olhar sensível sobre os processos de evolução, onde cada pequeno <span className="text-[#6E5483] font-medium">avanço</span> é celebrado como uma grande conquista.
                        </p>
                    </div>
                </div>

                {/* GRID DE FOTOS - Layout "Bento Grid" para 5 fotos */}
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 w-full z-10 auto-rows-[200px] md:auto-rows-[300px]">
                    {listaFotos.map((foto, index) => (
                        <div
                            key={index}
                            onClick={() => setFotoSelecionada(foto)}
                            className={`
                                group relative bg-white p-2 md:p-4 rounded-[24px] md:rounded-[40px] 
                                shadow-[0_10px_30px_-15px_rgba(85,60,106,0.1)] 
                                hover:shadow-[0_30px_60px_-15px_rgba(85,60,106,0.2)] 
                                transition-all duration-500 hover:-translate-y-2 cursor-pointer border border-gray-100 overflow-hidden
                        ...
                        >
                            ...
                            {/* Overlay Sutil com Roxo Eleven */}
                            <div className="absolute inset-2 md:inset-4 bg-[#553C6A]/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center p-4 text-center rounded-[18px] md:rounded-[30px] backdrop-blur-[2px]">
                                <Maximize2 size={24} className="text-white/70 mb-3 scale-50 group-hover:scale-100 transition-transform duration-500" />
            ...
            {fotoSelecionada !== null && (
                ...
                >
                    ...
                    >
                        ...
                        />
                        <p className="mt-8 text-white/90 text-lg md:text-2xl font-medium tracking-tight text-center max-w-2xl">
